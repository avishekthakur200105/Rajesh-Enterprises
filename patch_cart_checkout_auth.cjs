...
const cartRedirectSearch = `  if (!isAdmin) {
    return <Navigate to="/shop" replace />;
  }`;
const cartRedirectReplace = `  if (!isAdmin) {
    return (
...
cartCode = cartCode.replace(cartRedirectSearch, cartRedirectReplace);
...
const checkoutRedirectSearch = `  if (!isAdmin) {
    return <Navigate to="/shop" replace />;
  }`;
const checkoutRedirectReplace = `  if (!isAdmin) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-24 text-center">
        <h2 className="text-2xl font-bold text-stone-800 mb-4">Restricted Access</h2>
        <p className="text-stone-600 mb-8">Purchasing is currently restricted to administrators only.</p>
...
checkoutCode = checkoutCode.replace(checkoutRedirectSearch, checkoutRedirectReplace);
